...
LLDebugInfo::LLDebugInfo(llvm::Module *M, StringRef Filename, StringRef Directory)
    : M(M), DIB(*M) {
  M->addModuleFlag(
      Module::Warning, "Debug Info Version", (uint32_t) DEBUG_METADATA_VERSION);
  ...
  CU                   = DIB.createCompileUnit(
      dwarf::DW_LANG_lo_user, DebugSrcFile, "llvm-dbas", false, "", 0);
...
DIType *LLDebugInfo::addType(Type *Ty, llvm::Module *Mod) {
  ...
  switch (Ty->getTypeID()) {
    ...
    case Type::IntegerTyID: {
      ...
      return DIB.createBasicType("i" + std::to_string(IntTy->getBitWidth()),
                                 IntTy->getBitWidth(),
    ...
    case Type::StructTyID: {
      ...
      for (Type *ArgType : StructTy->elements()) {
        ...
        argTypes.push_back(DIB.createMemberType(
            File,
            std::to_string(i),
            File,
            0,
            layout.getTypeSizeInBits(ArgType),
            layout.getABITypeAlignment(ArgType),
            layout.getStructLayout(StructTy)->getElementOffsetInBits(i),
            DINode::FlagZero,
            DeclTy));
        i++;
      }
      return DIB.createStructType(File,
                                  StructTy->hasName() ? StructTy->getName() : "",
                                  File,
                                  0,
    ...
    case Type::ArrayTyID: {
      ...
      return DIB.createArrayType(layout.getTypeSizeInBits(Ty),
                                 layout.getABITypeAlignment(Ty),
                                 addType(ArrayTy->getElementType(), Mod),
...
void LLDebugInfo::addInstruction(Instruction *I,
                                 DISubprogram *SP,
                                 std::string NameStr,
                                 int NameID,
                                 unsigned int Line) {
