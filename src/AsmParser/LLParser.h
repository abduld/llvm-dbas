...
struct ValID {
  ...
  bool operator<(const ValID &RHS) const {
    ...
    assert((Kind == t_LocalName || Kind == t_GlobalName || Kind == t_ConstantStruct ||
            Kind == t_PackedConstantStruct) &&
...
private:
  ...
  std::map<unsigned, std::vector<std::pair<ValueInfo *, LocTy>>> ForwardRefValueInfos;
  std::map<unsigned, std::vector<std::pair<AliasSummary *, LocTy>>> ForwardRefAliasees;
...
public:
  LLParser(StringRef F, StringRef Directory, SourceMgr &SM, SMDiagnostic &Err, Module *M,
           ModuleSummaryIndex *Index, LLVMContext &Context, SlotMapping *Slots = nullptr,
           bool UpgradeDebugInfo = true, StringRef DataLayoutString = "")
      : Context(Context), Lex(F, SM, Err, Context), M(M), Index(Index), Slots(Slots),
        BlockAddressPFS(nullptr), UpgradeDebugInfo(UpgradeDebugInfo),
        DataLayoutStr(DataLayoutString), DebugInfo(M, F, Directory) {
...
private:
  ...
  GlobalValue *GetGlobalVal(const std::string &N, Type *Ty, LocTy Loc, bool IsCall);
  ...
  bool ParseOptionalProgramAddrSpace(unsigned &AddrSpace) {
    return ParseOptionalAddrSpace(AddrSpace, M->getDataLayout().getProgramAddressSpace());
  };
  bool ParseOptionalParamAttrs(AttrBuilder &B);
  bool ParseOptionalReturnAttrs(AttrBuilder &B);
  bool ParseOptionalLinkage(unsigned &Res, bool &HasLinkage, unsigned &Visibility,
  ...
  bool ParseOptionalCommaAddrSpace(unsigned &AddrSpace, LocTy &Loc, bool &AteExtraComma);
  bool ParseOptionalCommaInAlloca(bool &IsInAlloca);
  bool parseAllocSizeArguments(unsigned &BaseSizeArg, Optional<unsigned> &HowManyArg);
  ...
  bool parseIndirectSymbol(const std::string &Name, LocTy NameLoc, unsigned L,
                           unsigned Visibility, unsigned DLLStorageClass, bool DSOLocal,
  ...
  bool ParseFnAttributeValuePairs(AttrBuilder &B, std::vector<unsigned> &FwdRefAttrGrps,
  ...
  bool ParseConstVCallList(lltok::Kind Kind,
                           std::vector<FunctionSummary::ConstVCall> &ConstVCallList);
  using IdToIndexMapType = std::map<unsigned, std::vector<std::pair<unsigned, LocTy>>>;
  bool ParseConstVCall(FunctionSummary::ConstVCall &ConstVCall,
                       IdToIndexMapType &IdToIndexMap, unsigned Index);
  bool ParseVFuncId(FunctionSummary::VFuncId &VFuncId, IdToIndexMapType &IdToIndexMap,
                    unsigned Index);
  ...
  bool ParseType(Type *&Result, const Twine &Msg, LocTy &Loc, bool AllowVoid = false) {
    Loc = Lex.getLoc();
  ...
  bool ConvertValIDToValue(Type *Ty, ValID &ID, Value *&V, PerFunctionState *PFS,
                           bool IsCall);

  Value *checkValidVariableType(LocTy Loc, const Twine &Name, Type *Ty, Value *Val,
                                bool IsCall);
  ...
  bool ParseTypeAndBasicBlock(BasicBlock *&BB, LocTy &Loc, PerFunctionState &PFS);
  ...
  struct ParamInfo {
    ...
    Value *V;
    AttributeSet Attrs;
    ParamInfo(LocTy loc, Value *v, AttributeSet attrs) : Loc(loc), V(v), Attrs(attrs) {
    }
  };
  ...
  bool ParseOptionalOperandBundles(SmallVectorImpl<OperandBundleDef> &BundleList,
                                   PerFunctionState &PFS);

  bool ParseExceptionArgs(SmallVectorImpl<Value *> &Args, PerFunctionState &PFS);
  ...
  bool ParseValueAsMetadata(Metadata *&MD, const Twine &TypeMsg, PerFunctionState *PFS);
...
#include "llvm/IR/Metadata.def"
  ...
  int ParseInstruction(Instruction *&Inst, BasicBlock *BB, PerFunctionState &PFS);
  ...
  bool ParseUnaryOp(Instruction *&Inst, PerFunctionState &PFS, unsigned Opc, bool IsFP);
  ...
  bool ParseCall(Instruction *&Inst, PerFunctionState &PFS, CallInst::TailCallKind TCK);
