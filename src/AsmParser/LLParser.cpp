...
bool LLParser::parseStandaloneConstantValue(Constant *&C, const SlotMapping *Slots) {
...
void LLParser::restoreParsingState(const SlotMapping *Slots) {
  ...
  for (const auto &I : Slots->NamedTypes)
    NamedTypes.insert(std::make_pair(I.getKey(), std::make_pair(I.second, LocTy())));
  for (const auto &I : Slots->Types)
    NumberedTypes.insert(std::make_pair(I.first, std::make_pair(I.second, LocTy())));
}
...
bool LLParser::ValidateEndOfModule() {
  ...
  for (const auto &NT : NumberedTypes)
    if (NT.second.second.isValid())
      return Error(NT.second.second, "use of undefined type '%" + Twine(NT.first) + "'");
  ...
  for (StringMap<std::pair<Type *, LocTy>>::iterator I = NamedTypes.begin(),
    ...
    if (I->second.second.isValid())
      return Error(I->second.second, "use of undefined type named '" + I->getKey() + "'");

  if (!ForwardRefComdats.empty())
    return Error(ForwardRefComdats.begin()->second,
                 "use of undefined comdat '$" + ForwardRefComdats.begin()->first + "'");
  ...
  if (!ForwardRefValIDs.empty())
    return Error(ForwardRefValIDs.begin()->second.second,
                 "use of undefined value '@" + Twine(ForwardRefValIDs.begin()->first) +
                     "'");
...
bool LLParser::ParseModuleAsm() {
  ...
  std::string AsmStr;
  if (ParseToken(lltok::kw_asm, "expected 'module asm'") || ParseStringConstant(AsmStr))
    return true;
...
bool LLParser::ParseDefine() {
  ...
  unsigned int Line = Lex.getUIntVal() + 1;
  DISubprogram *SP  = DebugInfo.addFunction(F, Line);

  bool result = ParseOptionalFunctionMetadata(*F) || ParseFunctionBody(*F, SP);
...
bool LLParser::ParseOptionalUnnamedAddr(GlobalVariable::UnnamedAddr &UnnamedAddr) {
...
bool LLParser::ParseUnnamedGlobal() {
  ...
  if (ParseOptionalLinkage(Linkage, HasLinkage, Visibility, DLLStorageClass, DSOLocal) ||
      ParseOptionalThreadLocal(TLM) || ParseOptionalUnnamedAddr(UnnamedAddr))
    return true;

  if (Lex.getKind() != lltok::kw_alias && Lex.getKind() != lltok::kw_ifunc)
    return ParseGlobal(Name, NameLoc, Linkage, HasLinkage, Visibility, DLLStorageClass,
                       DSOLocal, TLM, UnnamedAddr);

  return parseIndirectSymbol(Name, NameLoc, Linkage, Visibility, DLLStorageClass,
                             DSOLocal, TLM, UnnamedAddr);
}
...
bool LLParser::ParseNamedGlobal() {
  ...
  if (ParseToken(lltok::equal, "expected '=' in global variable") ||
      ParseOptionalLinkage(Linkage, HasLinkage, Visibility, DLLStorageClass, DSOLocal) ||
      ParseOptionalThreadLocal(TLM) || ParseOptionalUnnamedAddr(UnnamedAddr))
    return true;
...
bool LLParser::ParseNamedMetadata() {
  ...
  if (Lex.getKind() != lltok::rbrace)
    do {
      ...
      if (Lex.getKind() == lltok::MetadataVar && Lex.getStrVal() == "DIExpression") {
        if (ParseDIExpression(N, /*IsDistinct=*/false))
          return true;
      } else if (ParseToken(lltok::exclaim, "Expected '!' here") || ParseMDNodeID(N)) {
        return true;
      }
      NMD->addOperand(N);
    } while (EatIfPresent(lltok::comma));
...
bool LLParser::SkipModuleSummaryEntry() {
  ...
  if (Lex.getKind() != lltok::kw_gv && Lex.getKind() != lltok::kw_module &&
    ...
    return TokError("Expected 'gv', 'module', or 'typeid' at the start of summary entry");
  Lex.Lex();
...
///
bool LLParser::parseIndirectSymbol(const std::string &Name, LocTy NameLoc, unsigned L,
  ...
  if (!isValidVisibilityForLinkage(Visibility, L))
    return Error(NameLoc, "symbol with local linkage must have default visibility");
  ...
  if (Lex.getKind() != lltok::kw_bitcast && Lex.getKind() != lltok::kw_getelementptr &&
      Lex.getKind() != lltok::kw_addrspacecast && Lex.getKind() != lltok::kw_inttoptr) {
  ...
  if (!IsAlias && !PTy->getElementType()->isFunctionTy())
    return Error(ExplicitTypeLoc, "explicit pointee type should be a function type");
  ...
  if (IsAlias)
    GA.reset(GlobalAlias::create(Ty, AddrSpace, (GlobalValue::LinkageTypes) Linkage, Name,
                                 Aliasee, /*Parent*/ nullptr));
  else
    GA.reset(GlobalIFunc::create(Ty, AddrSpace, (GlobalValue::LinkageTypes) Linkage, Name,
                                 Aliasee, /*Parent*/ nullptr));
...
/// already.
///
bool LLParser::ParseGlobal(const std::string &Name, LocTy NameLoc, unsigned Linkage,
                           bool HasLinkage, unsigned Visibility, unsigned DLLStorageClass,
                           bool DSOLocal, GlobalVariable::ThreadLocalMode TLM,
                           GlobalVariable::UnnamedAddr UnnamedAddr) {
  ...
  if (!HasLinkage ||
      !GlobalValue::isValidDeclarationLinkage((GlobalValue::LinkageTypes) Linkage)) {
  ...
  GlobalVariable *GV;
  if (!GVal) {
    GV = new GlobalVariable(*M, Ty, false, GlobalValue::ExternalLinkage, nullptr, Name,
...
bool LLParser::ParseUnnamedAttrGrp() {
  ...
  if (ParseToken(lltok::equal, "expected '=' here") ||
      ParseToken(lltok::lbrace, "expected '{' here") ||
      ParseFnAttributeValuePairs(NumberedAttrBuilders[VarID], unused, true, BuiltinLoc) ||
  ...
  while (true) {
    ...
    switch (Token) {
      ...
      case lltok::kw_align: {
        ...
        if (inAttrGrp) {
          Lex.Lex();
          if (ParseToken(lltok::equal, "expected '=' here") || ParseUInt32(Alignment))
            return true;
        } else {
          if (ParseOptionalAlignment(Alignment))
            return true;
...
static inline GlobalValue *createGlobalFwdRef(Module *M, PointerType *PTy,
  ...
  if (auto *FT = dyn_cast<FunctionType>(PTy->getElementType()))
    return Function::Create(FT, GlobalValue::ExternalWeakLinkage, PTy->getAddressSpace(),
                            Name, M);
  else
    return new GlobalVariable(*M, PTy->getElementType(), false,
                              GlobalValue::ExternalWeakLinkage, nullptr, Name, nullptr,
                              GlobalVariable::NotThreadLocal, PTy->getAddressSpace());
}
...
Value *LLParser::checkValidVariableType(LocTy Loc, const Twine &Name, Type *Ty,
  ...
  if (Ty->isLabelTy())
    ...
    Error(Loc, "'" + Name + "' defined with type '" + getTypeString(Val->getType()) +
                   "' but expected '" + getTypeString(SuggestedTy) + "'");
  return nullptr;
}
...
GlobalValue *LLParser::GetGlobalVal(const std::string &Name, Type *Ty, LocTy Loc,
                                    bool IsCall) {
  ...
  GlobalValue *Val = cast_or_null<GlobalValue>(M->getValueSymbolTable().lookup(Name));
...
GlobalValue *LLParser::GetGlobalVal(unsigned ID, Type *Ty, LocTy Loc, bool IsCall) {
  PointerType *PTy = dyn_cast<PointerType>(Ty);
  if (!PTy) {
    Error(Loc, "global variable reference must have pointer type");
    return nullptr;
...
///   ::= 'extern_weak'
///   ::= 'external'
bool LLParser::ParseOptionalLinkage(unsigned &Res, bool &HasLinkage, unsigned &Visibility,
...
bool LLParser::ParseOptionalDerefAttrBytes(lltok::Kind AttrKind, uint64_t &Bytes) {
...
/// end.
bool LLParser::ParseOptionalCommaAlign(unsigned &Alignment, bool &AteExtraComma) {
  AteExtraComma = false;
  while (EatIfPresent(lltok::comma)) {
...
bool LLParser::parseAllocSizeArguments(unsigned &BaseSizeArg,
  ...
  if (EatIfPresent(lltok::comma)) {
    ...
    if (HowMany == BaseSizeArg)
      return Error(HowManyAt, "'allocsize' indices can't refer to the same parameter");
    HowManyArg = HowMany;
  } else
    HowManyArg = None;
...
///    ::=  (',' uint32)+
///
bool LLParser::ParseIndexList(SmallVectorImpl<unsigned> &Indices, bool &AteExtraComma) {
  AteExtraComma = false;
...
bool LLParser::ParseParameterList(SmallVectorImpl<ParamInfo> &ArgList,
  ...
  while (Lex.getKind() != lltok::rparen) {
    ...
    if (!ArgList.empty() && ParseToken(lltok::comma, "expected ',' in argument list"))
      return true;
    ...
    ArgList.push_back(ParamInfo(ArgLoc, V, AttributeSet::get(V->getContext(), ArgAttrs)));
  }
...
bool LLParser::ParseOptionalOperandBundles(SmallVectorImpl<OperandBundleDef> &BundleList,
  ...
  while (Lex.getKind() != lltok::rsquare) {
    ...
    if (!BundleList.empty() && ParseToken(lltok::comma, "expected ',' in input list"))
      return true;
    ...
    while (Lex.getKind() != lltok::rparen) {
      ...
      if (!Inputs.empty() && ParseToken(lltok::comma, "expected ',' in input list"))
        return true;
...
///   ::= ArgType (',' ArgType)*
///
bool LLParser::ParseArgumentList(SmallVectorImpl<ArgInfo> &ArgList, bool &isVarArg) {
  isVarArg = false;
  ...
  if (Lex.getKind() == lltok::rparen) {
    ...
    ArgList.emplace_back(TypeLoc, ArgTy, AttributeSet::get(ArgTy->getContext(), Attrs),
                         std::move(Name));
...
bool LLParser::ParseFunctionType(Type *&Result) {
  ...
  for (unsigned i = 0, e = ArgList.size(); i != e; ++i) {
    ...
    if (ArgList[i].Attrs.hasAttributes())
      return Error(ArgList[i].Loc, "argument attributes invalid in function type");
  }
...
LLParser::PerFunctionState::PerFunctionState(LLParser &p, Function &f, int functionNumber,
...
LLParser::PerFunctionState::~PerFunctionState() {
  ...
  for (const auto &P : ForwardRefVals) {
    if (isa<BasicBlock>(P.second.first))
      continue;
    P.second.first->replaceAllUsesWith(UndefValue::get(P.second.first->getType()));
...
/// forward reference record if needed.  This can return null if the value
/// exists but does not have the right type.
Value *LLParser::PerFunctionState::GetVal(const std::string &Name, Type *Ty, LocTy Loc,
                                          bool IsCall) {
  // Look this name up in the normal function symbol table.
...
Value *LLParser::PerFunctionState::GetVal(unsigned ID, Type *Ty, LocTy Loc, bool IsCall) {
  // Look this name up in the normal function symbol table.
...
bool LLParser::PerFunctionState::SetInstName(int NameID, const std::string &NameStr,
  ...
  if (Inst->getName() != NameStr)
    return P.Error(NameLoc, "multiple definition of local value named '" + NameStr + "'");
  return false;
}

/// GetBB - Get a basic block with the specified name or ID, creating a
/// forward reference record if needed.
BasicBlock *LLParser::PerFunctionState::GetBB(const std::string &Name, LocTy Loc) {
...
BasicBlock *LLParser::PerFunctionState::DefineBB(const std::string &Name, int NameID,
                                                 LocTy Loc) {
  BasicBlock *BB;
  if (Name.empty()) {
    if (NameID != -1 && unsigned(NameID) != NumberedVals.size()) {
      P.Error(Loc, "label expected to be numbered '" + Twine(NumberedVals.size()) + "'");
      return nullptr;
    }
  ...
  switch (Lex.getKind()) {
    ...
    case lltok::less: {
      ...
      if (!Elts[0]->getType()->isIntegerTy() &&
          !Elts[0]->getType()->isFloatingPointTy() && !Elts[0]->getType()->isPointerTy())
      ...
      for (unsigned i = 1, e = Elts.size(); i != e; ++i)
        if (Elts[i]->getType() != Elts[0]->getType())
          return Error(FirstEltLoc,
                       "vector element #" + Twine(i) + " is not of type '" +
    ...
    case lltok::lsquare: { // Array Constant
      ...
      if (!Elts[0]->getType()->isFirstClassType())
        return Error(FirstEltLoc,
                     "invalid array element type: " + getTypeString(Elts[0]->getType()));
      ...
      for (unsigned i = 0, e = Elts.size(); i != e; ++i) {
        if (Elts[i]->getType() != Elts[0]->getType())
          return Error(FirstEltLoc,
                       "array element #" + Twine(i) + " is not of type '" +
                           getTypeString(Elts[0]->getType()));
      }
    ...
    case lltok::kw_c: // c "foo"
      Lex.Lex();
      ID.ConstantVal = ConstantDataArray::getString(Context, Lex.getStrVal(), false);
    ...
    case lltok::kw_blockaddress: {
      ...
      if (!F) {
        ...
        GlobalValue *&FwdRef =
            ForwardRefBlockAddresses
                .insert(std::make_pair(std::move(Fn), std::map<ValID, GlobalValue *>()))
      ...
      if (BlockAddressPFS && F == &BlockAddressPFS->getFunction()) {
        ...
        BB = dyn_cast_or_null<BasicBlock>(F->getValueSymbolTable()->lookup(Label.StrVal));
        if (!BB)
          return Error(Label.Loc, "referenced value is not a basic block");
      }
    ...
    case lltok::kw_ptrtoint: {
      ...
      ID.ConstantVal = ConstantExpr::getCast((Instruction::CastOps) Opc, SrcVal, DestTy);
      ID.Kind        = ValID::t_Constant;
      return false;
    }
    ...
    case lltok::kw_xor: {
      ...
      if (Val0->getType() != Val1->getType())
        return Error(ID.Loc, "operands of constexpr must have same type");
      if (!Val0->getType()->isIntOrIntVectorTy())
        return Error(ID.Loc, "constexpr requires integer or integer vector operands");
    ...
    case lltok::kw_select: {
      ...
      if (ParseGlobalValueVector(Elts, Opc == Instruction::GetElementPtr ? &InRangeOp
                                                                         : nullptr) ||
      ...
      if (Opc == Instruction::GetElementPtr) {
        ...
        unsigned GEPWidth = BaseType->isVectorTy() ? BaseType->getVectorNumElements() : 0;
        ...
        if (InRangeOp) {
          if (*InRangeOp == 0)
            return Error(ID.Loc, "inrange keyword may not appear on pointer operand");
          --*InRangeOp;
        }

        ID.ConstantVal =
            ConstantExpr::getGetElementPtr(Ty, Elts[0], Indices, InBounds, InRangeOp);
...
bool LLParser::ParseGlobalValue(Type *Ty, Constant *&C) {
  ...
  Value *V = nullptr;
  bool Parsed =
      ParseValID(ID) || ConvertValIDToValue(Ty, ID, V, nullptr, /*IsCall=*/false);
...
struct MDFieldImpl {
  ...
  explicit MDFieldImpl(FieldTy Default) : Val(std::move(Default)), Seen(false) {
  }
};
...
struct MDEitherFieldImpl {
  ...
  explicit MDEitherFieldImpl(FieldTypeA DefaultA, FieldTypeB DefaultB)
      : A(std::move(DefaultA)), B(std::move(DefaultB)), Seen(false), WhatIs(IsInvalid) {
  }
};
...
struct NameTableKindField : public MDUnsignedField {
  NameTableKindField()
      : MDUnsignedField(
            0, (unsigned) DICompileUnit::DebugNameTableKind::LastDebugNameTableKind) {
  }
};
...
struct MDSignedField : public MDFieldImpl<int64_t> {
  ...
  MDSignedField(int64_t Default = 0) : ImplTy(Default), Min(INT64_MIN), Max(INT64_MAX) {
  }
...
struct MDStringField : public MDFieldImpl<MDString *> {
  bool AllowEmpty;
  MDStringField(bool AllowEmpty = true) : ImplTy(nullptr), AllowEmpty(AllowEmpty) {
  }
};
...
struct MDSignedOrMDField : MDEitherFieldImpl<MDSignedField, MDField> {
  ...
  MDSignedOrMDField(int64_t Default, int64_t Min, int64_t Max, bool AllowNull = true)
...
struct MDSignedOrUnsignedField : MDEitherFieldImpl<MDSignedField, MDUnsignedField> {
...
template <>
bool LLParser::ParseMDField(LocTy Loc, StringRef Name, MDUnsignedField &Result) {
  if (Lex.getKind() != lltok::APSInt || Lex.getAPSIntVal().isSigned())
    return TokError("expected unsigned integer");

  auto &U = Lex.getAPSIntVal();
  if (U.ugt(Result.Max))
    return TokError("value for '" + Name + "' too large, limit is " + Twine(Result.Max));
...
template <>
bool LLParser::ParseMDField(LocTy Loc, StringRef Name, DwarfMacinfoTypeField &Result) {
  if (Lex.getKind() == lltok::APSInt)
    return ParseMDField(Loc, Name, static_cast<MDUnsignedField &>(Result));
  ...
  if (Macinfo == dwarf::DW_MACINFO_invalid)
    return TokError("invalid DWARF macinfo type" + Twine(" '") + Lex.getStrVal() + "'");
...
template <>
bool LLParser::ParseMDField(LocTy Loc, StringRef Name, DwarfVirtualityField &Result) {
  if (Lex.getKind() == lltok::APSInt)
    return ParseMDField(Loc, Name, static_cast<MDUnsignedField &>(Result));
  ...
  if (Virtuality == dwarf::DW_VIRTUALITY_invalid)
    return TokError("invalid DWARF virtuality code" + Twine(" '") + Lex.getStrVal() +
                    "'");
...
template <>
bool LLParser::ParseMDField(LocTy Loc, StringRef Name, NameTableKindField &Result) {
  if (Lex.getKind() == lltok::APSInt)
    return ParseMDField(Loc, Name, static_cast<MDUnsignedField &>(Result));
...
template <>
bool LLParser::ParseMDField(LocTy Loc, StringRef Name, DwarfAttEncodingField &Result) {
  if (Lex.getKind() == lltok::APSInt)
    return ParseMDField(Loc, Name, static_cast<MDUnsignedField &>(Result));
...
bool LLParser::ParseMDField(LocTy Loc, StringRef Name, DIFlagField &Result) {
  ...
  auto parseFlag = [&](DINode::DIFlags &Val) {
    ...
    if (!Val)
      return TokError(Twine("invalid debug info flag flag '") + Lex.getStrVal() + "'");
    Lex.Lex();
    return false;
  };
...
bool LLParser::ParseMDField(LocTy Loc, StringRef Name, DISPFlagField &Result) {
  ...
  auto parseFlag = [&](DISubprogram::DISPFlags &Val) {
    ...
    if (!Val)
      return TokError(Twine("invalid subprogram debug info flag '") + Lex.getStrVal() +
                      "'");
    Lex.Lex();
    return false;
  };
...
bool LLParser::ParseMDField(LocTy Loc, StringRef Name, MDSignedField &Result) {
  ...
  if (S < Result.Min)
    return TokError("value for '" + Name + "' too small, limit is " + Twine(Result.Min));
  if (S > Result.Max)
    return TokError("value for '" + Name + "' too large, limit is " + Twine(Result.Max));
...
template <>
bool LLParser::ParseMDField(LocTy Loc, StringRef Name, MDSignedOrMDField &Result) {
...
template <>
bool LLParser::ParseMDField(LocTy Loc, StringRef Name, MDSignedOrUnsignedField &Result) {
  if (Lex.getKind() != lltok::APSInt)
    return false;
...
template <>
bool LLParser::ParseMDField(LocTy Loc, StringRef Name, ChecksumKindField &Result) {
  Optional<DIFile::ChecksumKind> CSKind = DIFile::getChecksumKind(Lex.getStrVal());
...
#undef VISIT_MD_FIELDS
  ...
  Result = GET_OR_DISTINCT(DILocation, (Context, line.Val, column.Val, scope.Val,
...
#undef VISIT_MD_FIELDS
  ...
  Result = GET_OR_DISTINCT(GenericDINode, (Context, tag.Val, header.Val, operands.Val));
  return false;
}
...
#undef VISIT_MD_FIELDS
  ...
  if (count.isMDSignedField())
    Result =
        GET_OR_DISTINCT(DISubrange, (Context, count.getMDSignedValue(), lowerBound.Val));
  else if (count.isMDField())
    Result =
        GET_OR_DISTINCT(DISubrange, (Context, count.getMDFieldValue(), lowerBound.Val));
  else
    return true;
...
#undef VISIT_MD_FIELDS
  ...
  Result = GET_OR_DISTINCT(DIEnumerator, (Context, Value, isUnsigned.Val, name.Val));
...
#undef VISIT_MD_FIELDS
  ...
  Result = GET_OR_DISTINCT(DIBasicType, (Context, tag.Val, name.Val, size.Val, align.Val,
                                         encoding.Val, flags.Val));
  return false;
}
...
#undef VISIT_MD_FIELDS
  ...
  if (identifier.Val)
    if (auto *CT = DICompositeType::buildODRType(
            Context, *identifier.Val, tag.Val, name.Val, file.Val, line.Val, scope.Val,
            baseType.Val, size.Val, align.Val, offset.Val, flags.Val, elements.Val,
            runtimeLang.Val, vtableHolder.Val, templateParams.Val, discriminator.Val)) {
      Result = CT;
      return false;
    }
  ...
  // map.
  Result = GET_OR_DISTINCT(DICompositeType,
                           (Context, tag.Val, name.Val, file.Val, line.Val, scope.Val,
                            baseType.Val, size.Val, align.Val, offset.Val, flags.Val,
                            elements.Val, runtimeLang.Val, vtableHolder.Val,
                            templateParams.Val, identifier.Val, discriminator.Val));
  return false;
}
...
#undef VISIT_MD_FIELDS
  ...
  Result = GET_OR_DISTINCT(DISubroutineType, (Context, flags.Val, cc.Val, types.Val));
  return false;
}
...
#undef VISIT_MD_FIELDS
  ...
  if ((SPFlags & DISubprogram::SPFlagDefinition) && !IsDistinct)
    return Lex.Error(
        Loc, "missing 'distinct', required for !DISubprogram that is a Definition");
  Result = GET_OR_DISTINCT(DISubprogram,
                           (Context, scope.Val, name.Val, linkageName.Val, file.Val,
                            line.Val, type.Val, scopeLine.Val, containingType.Val,
                            virtualIndex.Val, thisAdjustment.Val, flags.Val, SPFlags,
                            unit.Val, templateParams.Val, declaration.Val,
                            retainedNodes.Val, thrownTypes.Val));
  return false;
}
...
#undef VISIT_MD_FIELDS
  ...
  Result = GET_OR_DISTINCT(DILexicalBlock,
                           (Context, scope.Val, file.Val, line.Val, column.Val));
  return false;
}
...
#undef VISIT_MD_FIELDS
  ...
  Result = GET_OR_DISTINCT(
      DICommonBlock, (Context, scope.Val, declaration.Val, name.Val, file.Val, line.Val));
  return false;
}
...
#undef VISIT_MD_FIELDS
  ...
  Result =
      GET_OR_DISTINCT(DINamespace, (Context, scope.Val, name.Val, exportSymbols.Val));
  return false;
}
...
#undef VISIT_MD_FIELDS
  ...
  Result = GET_OR_DISTINCT(DIMacro, (Context, type.Val, line.Val, name.Val, value.Val));
  return false;
}
...
#undef VISIT_MD_FIELDS
  ...
  Result =
      GET_OR_DISTINCT(DIMacroFile, (Context, type.Val, line.Val, file.Val, nodes.Val));
  return false;
}
...
#undef VISIT_MD_FIELDS
  ...
  Result = GET_OR_DISTINCT(DIModule, (Context, scope.Val, name.Val, configMacros.Val,
...
#undef VISIT_MD_FIELDS
  ...
  Result = GET_OR_DISTINCT(DITemplateTypeParameter, (Context, name.Val, type.Val));
  return false;
}
...
#undef VISIT_MD_FIELDS
  ...
  Result = GET_OR_DISTINCT(DILocalVariable,
                           (Context, scope.Val, name.Val, file.Val, line.Val, type.Val,
                            arg.Val, flags.Val, align.Val));
  return false;
}
...
#undef VISIT_MD_FIELDS
  ...
  Result = GET_OR_DISTINCT(DILabel, (Context, scope.Val, name.Val, file.Val, line.Val));
  return false;
}
...
bool LLParser::ParseDIGlobalVariableExpression(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                              \
...
#undef VISIT_MD_FIELDS
  ...
  Result = GET_OR_DISTINCT(DIGlobalVariableExpression, (Context, var.Val, expr.Val));
  return false;
}
...
#undef VISIT_MD_FIELDS
  ...
  Result = GET_OR_DISTINCT(DIObjCProperty,
                           (Context, name.Val, file.Val, line.Val, setter.Val, getter.Val,
...
bool LLParser::ConvertValIDToValue(Type *Ty, ValID &ID, Value *&V, PerFunctionState *PFS,
                                   bool IsCall) {
  if (Ty->isFunctionTy())
  ...
  switch (ID.Kind) {
    ...
    case ValID::t_APFloat:
      if (!Ty->isFloatingPointTy() || !ConstantFP::isValueValidForType(Ty, ID.APFloatVal))
    ...
    case ValID::t_PackedConstantStruct:
      if (StructType *ST = dyn_cast<StructType>(Ty)) {
        ...
        V = ConstantStruct::get(ST,
                                makeArrayRef(ID.ConstantStructElts.get(), ID.UIntVal));
      } else
        return Error(ID.Loc, "constant expression type mismatch");
      return false;
  }
...
bool LLParser::ParseValue(Type *Ty, Value *&V, PerFunctionState *PFS) {
  V = nullptr;
  ValID ID;
  return ParseValID(ID, PFS) || ConvertValIDToValue(Ty, ID, V, PFS, /*IsCall=*/false);
}
...
bool LLParser::ParseFunctionHeader(Function *&Fn, bool isDefine) {
  ...
  if (!isValidVisibilityForLinkage(Visibility, Linkage))
    return Error(LinkageLoc, "symbol with local linkage must have default visibility");
  ...
  if (Lex.getKind() == lltok::GlobalVar) {
    ...
    if (NameID != NumberedVals.size())
      return TokError("function expected to be numbered '%" + Twine(NumberedVals.size()) +
                      "'");
  } else {
  ...
  if (ParseArgumentList(ArgList, isVarArg) || ParseOptionalUnnamedAddr(UnnamedAddr) ||
      ParseOptionalProgramAddrSpace(AddrSpace) ||
      ParseFnAttributeValuePairs(FuncAttrs, FwdRefAttrGrps, false, BuiltinLoc) ||
      (EatIfPresent(lltok::kw_section) && ParseStringConstant(Section)) ||
      (EatIfPresent(lltok::kw_partition) && ParseStringConstant(Partition)) ||
      parseOptionalComdat(FunctionName, C) || ParseOptionalAlignment(Alignment) ||
      ...
      (EatIfPresent(lltok::kw_personality) && ParseGlobalTypeAndValue(PersonalityFn)))
    return true;
  ...
  AttributeList PAL = AttributeList::get(Context, AttributeSet::get(Context, FuncAttrs),
  ...
  if (!FunctionName.empty()) {
    ...
    if (FRVI != ForwardRefVals.end()) {
      ...
      return Error(NameLoc, "invalid redefinition of function '" + FunctionName + "'");
  ...
  if (!Fn)
    Fn = Function::Create(FT, GlobalValue::ExternalLinkage, AddrSpace, FunctionName, M);
  ...
  for (unsigned i = 0, e = ArgList.size(); i != e; ++i, ++ArgIt) {
    ...
    if (ArgIt->getName() != ArgList[i].Name)
      return Error(ArgList[i].Loc, "redefinition of argument '%" + ArgList[i].Name + "'");
  }
  ...
  if (Blocks != ForwardRefBlockAddresses.end())
    return Error(Blocks->first.Loc, "cannot take blockaddress inside a declaration");
  return false;
}
...
bool LLParser::ParseFunctionBody(Function &Fn, DISubprogram *SP) {
  ...
  while (Lex.getKind() != lltok::rbrace && Lex.getKind() != lltok::kw_uselistorder)
...
bool LLParser::ParseBasicBlock(PerFunctionState &PFS) {
  ...
  do {
    ...
    unsigned int InstLine = Lex.getUIntVal() + 1;
    ...
    switch (ParseInstruction(Inst, BB, PFS)) {
      ...
      case InstNormal:
        ...
        DebugInfo.addInstruction(Inst, PFS.getDebugSubprogram(), NameStr, NameID,
                                 InstLine);
...
bool LLParser::ParseRet(Instruction *&Inst, BasicBlock *BB, PerFunctionState &PFS) {
  SMLoc TypeLoc = Lex.getLoc();
  Type *Ty      = nullptr;
...
bool LLParser::ParseInvoke(Instruction *&Inst, PerFunctionState &PFS) {
  ...
  if (ParseOptionalCallingConv(CC) || ParseOptionalReturnAttrs(RetAttrs) ||
      ...
      ParseType(RetType, RetTypeLoc, true /*void allowed*/) || ParseValID(CalleeID) ||
      ParseParameterList(ArgList, PFS) ||
      ParseFnAttributeValuePairs(FnAttrs, FwdRefAttrGrps, false, NoBuiltinLoc) ||
  ...
  // Look up the callee.
  Value *Callee;
  if (ConvertValIDToValue(PointerType::get(Ty, InvokeAddrSpace), CalleeID, Callee, &PFS,
                          /*IsCall=*/true))
    return true;
  ...
  AttributeList PAL = AttributeList::get(Context, AttributeSet::get(Context, FnAttrs),
                                         AttributeSet::get(Context, RetAttrs), ArgAttrs);

  InvokeInst *II = InvokeInst::Create(Ty, Callee, NormalBB, UnwindBB, Args, BundleList);
...
bool LLParser::ParseExceptionArgs(SmallVectorImpl<Value *> &Args, PerFunctionState &PFS) {
  if (ParseToken(lltok::lsquare, "expected '[' in catchpad/cleanuppad"))
    return true;

  while (Lex.getKind() != lltok::rsquare) {
    // If this isn't the first argument, we need a comma.
    if (!Args.empty() && ParseToken(lltok::comma, "expected ',' in argument list"))
      return true;
...
bool LLParser::ParseCatchSwitch(Instruction *&Inst, PerFunctionState &PFS) {
  ...
  auto *CatchSwitch = CatchSwitchInst::Create(ParentPad, UnwindBB, Table.size());
...
/// operand is allowed.
bool LLParser::ParseUnaryOp(Instruction *&Inst, PerFunctionState &PFS, unsigned Opc,
                            bool IsFP) {
  LocTy Loc;
  Value *LHS;
  ...
  bool Valid =
      IsFP ? LHS->getType()->isFPOrFPVectorTy() : LHS->getType()->isIntOrIntVectorTy();
...
bool LLParser::ParseCallBr(Instruction *&Inst, PerFunctionState &PFS) {
  ...
  CallBrInst *CBI =
      CallBrInst::Create(Ty, Callee, DefaultDest, IndirectDests, Args, BundleList);
...
/// If IsFP is false, then any integer operand is allowed, if it is true, any fp
/// operand is allowed.
bool LLParser::ParseArithmetic(Instruction *&Inst, PerFunctionState &PFS, unsigned Opc,
                               bool IsFP) {
  LocTy Loc;
  Value *LHS, *RHS;
  if (ParseTypeAndValue(LHS, Loc, PFS) ||
...
/// ParseLogical
///  ::= ArithmeticOps TypeAndValue ',' Value {
bool LLParser::ParseLogical(Instruction *&Inst, PerFunctionState &PFS, unsigned Opc) {
  LocTy Loc;
  Value *LHS, *RHS;
  if (ParseTypeAndValue(LHS, Loc, PFS) ||
...
bool LLParser::ParseCompare(Instruction *&Inst, PerFunctionState &PFS, unsigned Opc) {
  ...
  if (Opc == Instruction::FCmp) {
    ...
    if (!LHS->getType()->isIntOrIntVectorTy() && !LHS->getType()->isPtrOrPtrVectorTy())
...
bool LLParser::ParseCast(Instruction *&Inst, PerFunctionState &PFS, unsigned Opc) {
  LocTy Loc;
  Value *Op;
  Type *DestTy = nullptr;
  if (ParseTypeAndValue(Op, Loc, PFS) ||
      ParseToken(lltok::kw_to, "expected 'to' after cast value") || ParseType(DestTy))
    return true;
...
bool LLParser::ParseCall(Instruction *&Inst, PerFunctionState &PFS,
  ...
  // Look up the callee.
  Value *Callee;
  if (ConvertValIDToValue(PointerType::get(Ty, CallAddrSpace), CalleeID, Callee, &PFS,
                          /*IsCall=*/true))
    return true;
...
int LLParser::ParseLoad(Instruction *&Inst, PerFunctionState &PFS) {
  ...
  Type *Ty;
  LocTy ExplicitTypeLoc = Lex.getLoc();
  if (ParseType(Ty) || ParseToken(lltok::comma, "expected comma after load's type") ||
...
int LLParser::ParseStore(Instruction *&Inst, PerFunctionState &PFS) {
  ...
  if (Ordering == AtomicOrdering::Acquire || Ordering == AtomicOrdering::AcquireRelease)
...
int LLParser::ParseCmpXchg(Instruction *&Inst, PerFunctionState &PFS) {
  ...
  AtomicCmpXchgInst *CXI =
      new AtomicCmpXchgInst(Ptr, Cmp, New, SuccessOrdering, FailureOrdering, SSID);
...
int LLParser::ParseAtomicRMW(Instruction *&Inst, PerFunctionState &PFS) {
  ...
  if (Operation == AtomicRMWInst::Xchg) {
    if (!Val->getType()->isIntegerTy() && !Val->getType()->isFloatingPointTy()) {
      return Error(ValLoc, "atomicrmw " + AtomicRMWInst::getOperationName(Operation) +
  ...
  while (EatIfPresent(lltok::comma)) {
    ...
    if (Val->getType()->isVectorTy()) {
      ...
      if (GEPWidth && GEPWidth != ValNumEl)
        return Error(EltLoc, "getelementptr vector index has a wrong number of elements");
      GEPWidth = ValNumEl;
    }
...
int LLParser::ParseExtractValue(Instruction *&Inst, PerFunctionState &PFS) {
  ...
  SmallVector<unsigned, 4> Indices;
  bool AteExtraComma;
  if (ParseTypeAndValue(Val, Loc, PFS) || ParseIndexList(Indices, AteExtraComma))
    return true;
...
int LLParser::ParseInsertValue(Instruction *&Inst, PerFunctionState &PFS) {
  ...
  if (ParseTypeAndValue(Val0, Loc0, PFS) ||
      ...
      ParseTypeAndValue(Val1, Loc1, PFS) || ParseIndexList(Indices, AteExtraComma))
    return true;
...
//===----------------------------------------------------------------------===//
bool LLParser::sortUseListOrder(Value *V, ArrayRef<unsigned> Indexes, SMLoc Loc) {
  if (V->use_empty())
  ...
  if (Order.size() != Indexes.size() || NumUses > Indexes.size())
    return Error(Loc, "wrong number of indexes, expected " + Twine(V->getNumUses()));

  V->sortUseList(
      [&](const Use &L, const Use &R) { return Order.lookup(&L) < Order.lookup(&R); });
  return false;
}
...
bool LLParser::ParseModuleEntry(unsigned ID) {
  ...
  if (ParseToken(lltok::colon, "expected ':' here") ||
      ...
      ParseToken(lltok::colon, "expected ':' here") || ParseStringConstant(Path) ||
...
bool LLParser::ParseTypeIdEntry(unsigned ID) {
  ...
  if (ParseToken(lltok::colon, "expected ':' here") ||
      ...
      ParseToken(lltok::colon, "expected ':' here") || ParseStringConstant(Name))
    return true;

  TypeIdSummary &TIS = Index->getOrInsertTypeIdSummary(Name);
  if (ParseToken(lltok::comma, "expected ',' here") || ParseTypeIdSummary(TIS) ||
      ParseToken(lltok::rparen, "expected ')' here"))
    return true;
  ...
  if (FwdRefTIDs != ForwardRefTypeIds.end()) {
    for (auto TIDRef : FwdRefTIDs->second) {
      assert(!*TIDRef.first && "Forward referenced type id GUID expected to be 0");
...
static ValueInfo EmptyVI = ValueInfo(false, (GlobalValueSummaryMapTy::value_type *) -8);
...
bool LLParser::ParseTypeIdCompatibleVtableEntry(unsigned ID) {
  ...
  TypeIdCompatibleVtableInfo &TI = Index->getOrInsertTypeIdCompatibleVtableSummary(Name);
  ...
  for (auto I : IdToIndexMap) {
    for (auto P : I.second) {
      ...
      auto FwdRef = ForwardRefValueInfos.insert(
          std::make_pair(I.first, std::vector<std::pair<ValueInfo *, LocTy>>()));
      FwdRef.first->second.push_back(std::make_pair(&TI[P.first].VTableVI, P.second));
    }
  }
...
bool LLParser::ParseTypeTestResolution(TypeTestResolution &TTRes) {
  ...
  if (ParseToken(lltok::comma, "expected ',' here") ||
      ...
      ParseToken(lltok::colon, "expected ':' here") || ParseUInt32(TTRes.SizeM1BitWidth))
    return true;
  ...
  while (EatIfPresent(lltok::comma)) {
    switch (Lex.getKind()) {
      case lltok::kw_alignLog2:
        Lex.Lex();
        if (ParseToken(lltok::colon, "expected ':'") || ParseUInt64(TTRes.AlignLog2))
          return true;
        break;
      ...
      case lltok::kw_inlineBits:
        Lex.Lex();
        if (ParseToken(lltok::colon, "expected ':'") || ParseUInt64(TTRes.InlineBits))
          return true;
        break;
      default:
  ...
  do {
    ...
    switch (Lex.getKind()) {
      ...
      case lltok::kw_virtualConstProp:
        ...
        break;
      default:
        return Error(Lex.getLoc(), "unexpected WholeProgramDevirtResolution::ByArg kind");
    }
    Lex.Lex();
    ...
    while (EatIfPresent(lltok::comma)) {
      switch (Lex.getKind()) {
        case lltok::kw_info:
          Lex.Lex();
          if (ParseToken(lltok::colon, "expected ':' here") || ParseUInt64(ByArg.Info))
            return true;
          break;
        case lltok::kw_byte:
          Lex.Lex();
          if (ParseToken(lltok::colon, "expected ':' here") || ParseUInt32(ByArg.Byte))
            return true;
          break;
        case lltok::kw_bit:
          Lex.Lex();
          if (ParseToken(lltok::colon, "expected ':' here") || ParseUInt32(ByArg.Bit))
            return true;
          break;
        default:
          return Error(Lex.getLoc(), "expected optional whole program devirt field");
      }
    }
...
void LLParser::AddGlobalValueToIndex(std::string Name, GlobalValue::GUID GUID,
...
bool LLParser::ParseGVEntry(unsigned ID) {
  ...
  switch (Lex.getKind()) {
    case lltok::kw_name:
      Lex.Lex();
      if (ParseToken(lltok::colon, "expected ':' here") || ParseStringConstant(Name))
        return true;
  ...
  if (!EatIfPresent(lltok::comma)) {
    ...
    AddGlobalValueToIndex(Name, GUID, GlobalValue::ExternalLinkage, ID, nullptr);
    return false;
  }
...
bool LLParser::ParseFunctionSummary(std::string Name, GlobalValue::GUID GUID,
  ...
  if (ParseToken(lltok::colon, "expected ':' here") ||
      ParseToken(lltok::lparen, "expected '(' here") ||
      ParseModuleReference(ModulePath) || ParseToken(lltok::comma, "expected ',' here") ||
      ParseGVFlags(GVFlags) || ParseToken(lltok::comma, "expected ',' here") ||
  ...
  auto FS = llvm::make_unique<FunctionSummary>(
      GVFlags, InstCount, FFlags, /*EntryCount=*/0, std::move(Refs), std::move(Calls),
      std::move(TypeIdInfo.TypeTests), std::move(TypeIdInfo.TypeTestAssumeVCalls),
  ...
  AddGlobalValueToIndex(Name, GUID, (GlobalValue::LinkageTypes) GVFlags.Linkage, ID,
                        std::move(FS));
...
bool LLParser::ParseVariableSummary(std::string Name, GlobalValue::GUID GUID,
  ...
  auto GS = llvm::make_unique<GlobalVarSummary>(GVFlags, GVarFlags, std::move(Refs));
...
///         'aliasee' ':' GVReference ')'
bool LLParser::ParseAliasSummary(std::string Name, GlobalValue::GUID GUID, unsigned ID) {
...
bool LLParser::ParseOptionalCalls(std::vector<FunctionSummary::EdgeTy> &Calls) {
  ...
  for (auto I : IdToIndexMap) {
    for (auto P : I.second) {
      ...
      auto FwdRef = ForwardRefValueInfos.insert(
          std::make_pair(I.first, std::vector<std::pair<ValueInfo *, LocTy>>()));
      FwdRef.first->second.push_back(std::make_pair(&Calls[P.first].first, P.second));
    }
  }
...
bool LLParser::ParseOptionalTypeIdInfo(FunctionSummary::TypeIdInfo &TypeIdInfo) {
...
bool LLParser::ParseTypeTests(std::vector<GlobalValue::GUID> &TypeTests) {
  ...
  for (auto I : IdToIndexMap) {
    for (auto P : I.second) {
      ...
      auto FwdRef = ForwardRefTypeIds.insert(
          std::make_pair(I.first, std::vector<std::pair<GlobalValue::GUID *, LocTy>>()));
      FwdRef.first->second.push_back(std::make_pair(&TypeTests[P.first], P.second));
    }
  }
...
bool LLParser::ParseVFuncIdList(lltok::Kind Kind,
...
bool LLParser::ParseConstVCallList(
    lltok::Kind Kind, std::vector<FunctionSummary::ConstVCall> &ConstVCallList) {
  assert(Lex.getKind() == Kind);
  Lex.Lex();
...
bool LLParser::ParseVFuncId(FunctionSummary::VFuncId &VFuncId,
  ...
  if (Lex.getKind() == lltok::SummaryID) {
    ...
    Lex.Lex();
             ...
             ParseToken(lltok::colon, "expected ':' here") || ParseUInt64(VFuncId.GUID))
    return true;

  if (ParseToken(lltok::comma, "expected ',' here") ||
      ParseToken(lltok::kw_offset, "expected 'offset' here") ||
      ParseToken(lltok::colon, "expected ':' here") || ParseUInt64(VFuncId.Offset) ||
      ParseToken(lltok::rparen, "expected ')' here"))
    return true;
